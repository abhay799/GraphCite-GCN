...
async function predictCoraNode(nodeId) {
  ...
  try {
    ...
    drawCitationGraph(nodeId, topicCfg.color, pred.neighbors || []);
...
function drawCitationGraph(centralNodeId, nodeColor, neighborIds) {
  ...
  const visibleNeighborIds = neighborIds.slice(0, 12);
  const radius = 88;

  const neighbors = visibleNeighborIds.map((id, i) => {
    const angle = (i * 2 * Math.PI) / Math.max(visibleNeighborIds.length, 1);
    return {
  ...
  if (neighbors.length === 0) {
    ...
    ctx.fillText("No 1-hop neighbors", centerX, centerY + 48);
  }
}
...
async function initModelInfo() {
  ...
  try {
    ...
    const data = await res.json();
    const modelName = data.model_name || "SimpleGCN";
